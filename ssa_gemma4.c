...
#define SSA_MAX_P 8
#define SSA_CAUSAL 1
#define SSA_LSH_C (SSA_K * 4)
#define SSA_MAX_CALL (SSA_ROUNDS * SSA_LSH_C)
...
typedef struct {
    ...
    // SSA scratch space
    float *ssa_proj;       // lazily allocated [NUM_LAYERS][SSA_ROUNDS][SSA_MAX_P][head_dim_max]
    float *ssa_scratch;    // lazily allocated scratch for graph construction
    size_t ssa_scratch_size;
    int ssa_proj_ready;    // nonzero once projections are generated
    int ssa_layer_hashed;  // tracks which layers already hashed this forward
} InferenceState;
...
static void ssa_generate_projections(InferenceState *state) {
    if (state->ssa_proj) return;
    size_t bytes = (size_t)NUM_LAYERS * SSA_ROUNDS * SSA_MAX_P * 512 * sizeof(float);
    state->ssa_proj = malloc(bytes);
    if (!state->ssa_proj) { fprintf(stderr, "OOM ssa_proj\n"); exit(1); }
    unsigned int seed = 12345;
    for (int layer = 0; layer < NUM_LAYERS; layer++) {
        for (int r = 0; r < SSA_ROUNDS; r++) {
            for (int p = 0; p < SSA_MAX_P; p++) {
                float *proj = state->ssa_proj + ((size_t)layer * SSA_ROUNDS + r) * SSA_MAX_P * 512 + (size_t)p * 512;
                float norm = 0.0f;
                for (int d = 0; d < 512; d++) {
                    seed = seed * 1664525u + 1013904223u;
                    float v = ((float)(seed >> 16) / 32768.0f) * 2.0f - 1.0f;
                    proj[d] = v;
                    norm += v * v;
                }
                norm = sqrtf(norm);
                if (norm > 0.0f) {
                    for (int d = 0; d < 512; d++) proj[d] /= norm;
                }
            }
        }
    }
...
// Builds queries, updates the KV cache, and computes sparse attention over a learned neighbor graph while shared layers reuse the latest compatible cache.
void attention(InferenceState *state, const LayerWeights *layers, int layer,
    ...
    rmsnorm(state->auxiliary, state->auxiliary, &weights->q_norm, head_dim, 1e-6f, token_count * num_heads);
    apply_rope(&weights->rope_cos, &weights->rope_sin, state->auxiliary, num_heads, head_dim, start_pos, token_count);
    ...
    // Generate LSH projections lazily.
    if (!state->ssa_proj) ssa_generate_projections(state);
    ...
    char *scratch = (char *)state->ssa_scratch;
    int *k_ids = (int *)scratch;
    int *perm = k_ids + M;
    int *sorted_k = perm + M;
    ...
    for (size_t head = 0; head < num_heads; head++) {
        const float *queries = state->auxiliary; // [token_count, query_width]
        int q_stride = query_width;
        float scale = 1.0f / sqrtf((float)head_dim);
        int causal = !full_attention;
        int window = SSA_WINDOW;
        int G = SSA_GLOBAL;
        int K = SSA_K;
        int R = SSA_ROUNDS;
        int max_P = SSA_MAX_P;
        int Call = SSA_MAX_CALL;

        int guaranteed = 2 * window + 1 + G + 1;
        int lsh_k = K - guaranteed;
        if (lsh_k < 8) lsh_k = 8;
        int lsh_candidates = lsh_k * 4;
        if (lsh_candidates > Call) lsh_candidates = Call;

        int p_eff = 1;
        if ((int)M > 0 && lsh_candidates > 0) {
            float p_ideal = log2f((float)M / (float)lsh_candidates);
            p_eff = (int)roundf(p_ideal);
            if (p_eff < 1) p_eff = 1;
            if (p_eff > max_P) p_eff = max_P;
        }
        int num_buckets = 1 << p_eff;
        if (num_buckets > 256) num_buckets = 256;
        ...
        for (int r = 0; r < R; r++) {
            for (size_t m = 0; m < M; m++) {
                ...
                for (int p = 0; p < p_eff; p++) {
                    float dot = 0.0f;
                    for (int d = 0; d < head_dim; d++) dot += kv[d] * ((float *)state->ssa_proj)[((size_t)layer * R + r) * max_P * head_dim + (size_t)p * head_dim + d];
                    if (dot > 0.0f) id |= (1 << p);
                }
                k_ids[m] = id;
            }
            ...
            for (size_t n = 0; n < token_count; n++) {
                ...
                for (int p = 0; p < p_eff; p++) {
                    float dot = 0.0f;
                    for (int d = 0; d < head_dim; d++) dot += qv[d] * ((float *)state->ssa_proj)[((size_t)layer * R + r) * max_P * head_dim + (size_t)p * head_dim + d];
                    if (dot > 0.0f) id |= (1 << p);
                }
                k_ids[M + n] = id;
            }
            ...
            for (size_t m = 0; m < M; m++) {
                int bid = k_ids[m];
                if (bid >= 0 && bid < num_buckets) count[bid]++;
            }
            int start[256] = {0};
            ...
            for (size_t m = 0; m < M; m++) {
                int bid = k_ids[m];
                if (bid >= 0 && bid < num_buckets) {
                    perm[start[bid]++] = (int)m;
                }
            }
            for (size_t m = 0; m < M; m++) sorted_k[m] = k_ids[perm[m]];
            ...
            for (size_t m = 0; m < M; m++) {
                int bid = sorted_k[m];
                if (bid >= 0 && bid < num_buckets) {
                    if ((int)m < bstart[bid]) bstart[bid] = (int)m;
                    bsize[bid]++;
                }
            }
            ...
            for (size_t n = 0; n < token_count; n++) {
                ...
                for (int c = 0; c < lsh_candidates; c++) {
                    ...
                    int cand_key = (cand_pos < (int)M) ? perm[cand_pos] : 0;
                    if (causal && valid_c && cand_key > (int)(start_pos + n)) {
                        valid_c = 0;
                        cand_key = (int)(start_pos + n);
                    }
                    all_cand[base + c] = cand_key;
                    all_valid[base + c] = (uint8_t)(valid_c ? 1 : 0);
                }
            }
            call_used += lsh_candidates;
        }
        ...
        for (size_t n = 0; n < token_count; n++) {
            ...
            for (int i = 0; i < total - 1; i++) {
                ...
                for (int j = i + 1; j < total; j++) {
                    if (all_cand[base + j] < all_cand[base + min_j]) min_j = j;
                }
                if (min_j != i) {
                    int tmp = all_cand[base + i];
                    all_cand[base + i] = all_cand[base + min_j];
                    all_cand[base + min_j] = tmp;
                    uint8_t vt = all_valid[base + i];
                    all_valid[base + i] = all_valid[base + min_j];
                    all_valid[base + min_j] = vt;
                }
            }
            uint8_t *first_valid = valid; // reuse valid buffer
            memset(first_valid, 0, total * sizeof(uint8_t));
            for (int i = 0; i < total; i++) {
                if (i == 0 || all_cand[base + i] != all_cand[base + i - 1]) {
                    first_valid[i] = all_valid[base + i];
                }
            }
            for (int i = 0; i < total; i++) {
                all_valid[base + i] = first_valid[i];
            }
        }
        ...
        // Exact rescore top-K
        {
            ...
            for (size_t n = 0; n < token_count; n++) {
                ...
                for (int c = 0; c < total; c++) {
                    ...
                    if (!all_valid[base_cand + c] || ki < 0 || ki >= (int)M) {
                        scores_buf[c] = -1e30f;
                        continue;
                    }
                    const float *kv = key_cache + (size_t)ki * head_dim;
                    float dot = 0.0f;
                    for (int d = 0; d < head_dim; d++) dot += qv[d] * kv[d];
                    scores_buf[c] = dot * scale;
                }
                for (int i = 0; i < actual_k; i++) {
                    int max_j = i;
                    for (int j = i + 1; j < total; j++) {
                        if (scores_buf[j] > scores_buf[max_j]) max_j = j;
                    }
                    if (max_j != i) {
                        float tmp = scores_buf[i];
                        scores_buf[i] = scores_buf[max_j];
                        scores_buf[max_j] = tmp;
                        int itmp = all_cand[base_cand + i];
                        all_cand[base_cand + i] = all_cand[base_cand + max_j];
                        all_cand[base_cand + max_j] = itmp;
                        uint8_t vt = all_valid[base_cand + i];
                        all_valid[base_cand + i] = all_valid[base_cand + max_j];
                        all_valid[base_cand + max_j] = vt;
                    }
                }
                int base_out = (int)n * K;
                for (int k = 0; k < K; k++) {
                    if (k < actual_k) {
                        neighbors[base_out + k] = all_cand[base_cand + k];
                        valid[base_out + k] = all_valid[base_cand + k];
                    } else {
                        neighbors[base_out + k] = 0;
                        valid[base_out + k] = 0;
                    }
                }
                for (int k = 0; k < K; k++) {
                    if (!valid[base_out + k]) {
                        neighbors[base_out + k] = (k < G) ? k : 0;
                        valid[base_out + k] = 1;
                    }
                }
            }
        ...
        // Merge window + global + LSH + self
        {
            ...
            for (size_t n = 0; n < token_count; n++) {
                ...
                for (int g = 0; g < G && g < (int)M; g++) {
                    combined[base + slot] = g;
                    combined_valid[base + slot] = 1;
                    slot++;
                }
                ...
                for (int i = 0; i < Kall; i++) {
                    if (i == 0 || combined[base + i] != combined[base + i - 1]) {
                        keep[i] = combined_valid[base + i] ? 1 : 0;
                    } else {
                        keep[i] = 0;
                    }
                }
                ...
                for (int i = 0; i < Kall; i++) {
                    if (keep[i]) {
                        combined[base + write] = combined[base + i];
                        write++;
                    }
                }
                for (int i = write; i < Kall; i++) {
                    combined[base + i] = sentinel;
                }
                for (int i = 1; i < Kall; i++) {
                    int v = combined[base + i];
                ...
                int base_out = (int)n * K;
                neighbors[base_out + 0] = (int)(start_pos + n);
                valid[base_out + 0] = 1;
                for (int k = 1; k < K; k++) {
                    int src = k - 1;
                    if (src < Kall && combined[base + src] < (int)M) {
                        neighbors[base_out + k] = combined[base + src];
                        valid[base_out + k] = 1;
                    } else {
                        neighbors[base_out + k] = 0;
                        valid[base_out + k] = 0;
        ...
        for (size_t n = 0; n < token_count; n++) {
            const float *qv = queries + n * q_stride + head * head_dim;
            float *out_ptr = state->hidden + n * query_width + head * head_dim;
            float attn_w[SSA_K];
            float max_score = -1e30f;
            int base_nb = (int)n * K;
            for (int k = 0; k < K; k++) {
                int ki = neighbors[base_nb + k];
                if (!valid[base_nb + k] || ki < 0 || ki >= (int)M) {
                    attn_w[k] = -1e30f;
                    continue;
                }
                const float *kv = key_cache + (size_t)ki * head_dim;
                ...
                for (int d = 0; d < head_dim; d++) dot += qv[d] * kv[d];
                attn_w[k] = dot * scale;
                if (attn_w[k] > max_score) max_score = attn_w[k];
            }
            float sum = 0.0f;
            for (int k = 0; k < K; k++) {
                attn_w[k] = expf(attn_w[k] - max_score);
                sum += attn_w[k];
            }
            if (sum > 0.0f) {
            ...
            for (int k = 0; k < K; k++) {
                ...
                const float *vv = value_cache + (size_t)ki * head_dim;
                float w = attn_w[k];
                for (int d = 0; d < head_dim; d++) out_ptr[d] += w * vv[d];
            }
        }
    }
